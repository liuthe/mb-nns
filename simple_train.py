...
model = spinsep_NNB(sys_size = sys_size, 
            hidden_size = hidden_size, 
            num_fillings = num_fillings)
#model  = spinsum_NNB(sys_size = sys_size,
#            hidden_size = hidden_size,
#            num_fillings = num_fillings)
...
for epoch in range(epochs):
    for i in range(train_iter):
        ...
        samples, prob = generate_sample(model, n_samples, n_chains, sweep_size = sweep_size)
        loc_e = h_model.local_energy(samples, model, train = True)
        # Compute the loss
        loss = loc_e
        energy, var_e = h_model.local_energy(samples, model, train = False)
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
