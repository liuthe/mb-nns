...
class NNB(nn.Module):
    ...
    def __init__(self, sys_size, hidden_size, output_size, dropout_rate=0.2):
        """ Init NNB Model.

        @param sys_size (int): System size (dimensionality)
        @param hidden_size (int): Hidden Size, the size of hidden states (dimensionality)
        @param output_size (int): Output Size, the size of output states (dimensionality)
        @param dropout_rate (float): Dropout probability, for attention
        """
        super(NNB, self).__init__()

        self.input_size = 2 * sys_size
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        embedded_X = self.model_embeddings.source(source_padded)
        embedded_X = embedded_X.data.permute(1, 2, 0)
        conv_out = self.post_embed_cnn(embedded_X)
        conv_out = conv_out.permute(2, 0, 1)
        packed_X = pack_padded_sequence(conv_out, source_lengths)
        encode_X, (last_hidden, last_cell) = self.encoder(packed_X)
        enc_hiddens, _ = pad_packed_sequence(encode_X, batch_first=True)
